<template>
	<li class="border border-base-content p-6 shadow-sm rounded-lg backdrop-blur-sm flex flex-col">
		...
		<div class="flex items-center justify-between">
			<p class="btn btn-soft btn-disabled btn-xs h-8 text-xs" v-if="isNotSource">
				<Icon name="bx:bxl-github" size="1.25rem" />
				{{ $t('projects.source') }}
			</p>
			<nuxt-link :to="project.source" target="_blank" class="btn btn-soft btn-xs h-8 text-xs" v-else>
				<Icon name="bx:bxl-github" size="1.25rem" />
				{{ $t('projects.source') }}
			</nuxt-link>
