export default {
  pages: {
    home: 'Home',
    blog: 'Blog',
  },
  home: {
    'part-time': 'Available for part-time',
    about: [
      'Howdy, I am Alexandr. I am building web applications using',
      'Connect with me for collaboration, part-time job, or just say hi',
    ],
  },
  projects: {
    title: 'Projects',
    source: 'Source code',
    website: 'Visit website',
    inDev: 'In development',
    soon: 'More coming soon',
  },
  blog: {
    title: 'Blog',
    desc: 'The place where I am sharing my thoughts, experience, knowledge',
    posts: 'Recent posts',
    views: ['No views', '{n} view', '{n} views'],
    likes: ['No likes', '{n} like', '{n} likes'],
    btn: {
      'was-helpful': 'This was helpful',
      thanks: 'Thanks',
    },
  },
  misc: {
    and: 'and',
  },
  settings: {
    title: 'settings',
    lang: 'language',
    theme: {
      title: 'theme',
      dark: 'dark',
      light: 'light',
      system: 'system',
    },
  },
}
